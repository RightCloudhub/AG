...
**更新日期：** 2026-07-24
**定位：** 描述性文档 — 模块地图、查询生命周期、离线/在线双轨、边界规则，以及 2026-07-23 静态架构审计的结论与优化建议（P-A1/P-A2 已实施）。强制性规则以 [`plan/engineering/rules.md`](../plan/engineering/rules.md) 为准；债务与延期以 [`IMPORTANT.md`](./IMPORTANT.md) 为准。
...
| 4 | `os.environ` 直读散布 | 集中在 `api/`；truthy 习语已归并到 `api/env_flags.py`（P-A1，2026-07-24） |
| 5 | 内联魔法默认值 | 限流 / host / port 默认值已提取具名常量（P-A2，2026-07-24） |
...
| ID | 建议 | 状态 | 要点 |
|---|---|---|---|
| P-A1 | 归并 truthy env 习语：`api/env_flags.py` 的 `env_flag(name)`（调用时读 env） | **已实施**（2026-07-24） | 保留 `require_auth_enabled` / `trust_x_user_id_enabled` 导入面；`service.py` 用 `env_flag as _env_flag` |
| P-A2 | `auth.py` 限流默认值、`app.py` host/port 提取具名常量 | **已实施**（2026-07-24） | `_DEFAULT_RATE_LIMIT_QPS/CONCURRENT`、`_DEFAULT_API_HOST/PORT` |
| P-A3 | `stores/neo4j_store.py` 已顶格 300 行：**下次任何改动前先拆分** | 未实施 | 规则要求拆分而非豁免；先例 `neo4j_codec.py`；建议按读/写或查询构造拆 |
| P-A4 | `config.py` 294 行：预留拆分方案（如 `config_paths.py`） | 未实施 | 无需立即动 |
| P-A5 | 长期：评估把 `AGR_*` 收进 pydantic-settings `Settings` | 未实施 / 低优先 | 会失去调用时读取语义；若收编需连带改测试，收益有限 |

## 8. 验证清单

- [x] `ruff check` + `ruff format --check`（P-A1/P-A2 改动后）
- [x] `pytest tests/unit/test_security_budget_cache_config.py` + 相关 API auth 单测
- [x] `python scripts/check_code_metrics.py`
- [ ] 全量 `pytest tests/unit --cov=agentic_graphrag --cov-fail-under=80 -q`（CI 门禁）
