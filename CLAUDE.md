...
- **Binding rules are consolidated in `plan/engineering/rules.md`** (code metrics, security checklist, architecture boundaries, frontend zero-build + Vue 3 pin ADR-006, docs-sync duties) with their enforcement mechanism (CI / gate script / review). Design-vs-implementation divergences are marked "⚠ 差异" in workstream docs and ledgered in `docs/IMPORTANT.md`. SSE is true incremental via LangGraph `stream(updates)` (`agent/loop_stream.py` + `api/service_stream.py`). Trial UI (`web/`): Vue 3 ESM runtime (vendored-first, no npm); mustache/`textContent` only — no `v-html`/`.innerHTML`; JS modules follow the same size limits as Python (review-enforced).
