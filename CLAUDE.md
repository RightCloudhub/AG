...
- **Binding rules are consolidated in `plan/engineering/rules.md`** (code metrics, security checklist, architecture boundaries, frontend zero-build rule, docs-sync duties) with their enforcement mechanism (CI / gate script / review). Design-vs-implementation divergences are marked "⚠ 差异" in workstream docs and ledgered — e.g. `/v1/query/stream` currently replays steps after the run completes rather than streaming live from LangGraph.
