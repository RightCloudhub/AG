...
**任务 ID**：P5-UI-01 · **版本**：V0.2（2026-07-21）· **状态**：**[x] 已完成** — 组件层/ CSS 三分拆/测试重写/docs 同步均落地；§7 验证清单待人工执行。
...
所有任务 R0–R13 已完成（详见 §5 表）。`web/` 可运行（需 vendor Vue 或 CDN 可访）；§7 验证清单待人工执行。回滚命令不再需要。
...
| `web/static/js/components/index.js` | 全局组件注册 | ~15 | **[x] 已落地** |
| `web/static/js/components/widgets.js` | progress-log 组件 | ~50 | **[x] 已落地** |
| `web/static/js/components/answer-turn.js` | 答案卡组件（角标/论断/反馈/折叠/复制/重试） | ~250 | **[x] 已落地** |
| `web/static/{app,chat,panels}.css` | 拆分：tokens+壳 / 线程+composer+进度 / 反馈+折叠+引用+树+路径 | 各 ≤300 | **[x] 已落地** |
| `web/static/vendor/README.md` | vendor 说明（钉版 URL + 提交入库建议） | ~40 | **[x] 已落地** |
...
- [x] **R1** `components/index.js`：`registerComponents(app)` 注册 `answer-turn` / `progress-log`
- [x] **R2** `components/widgets.js`：ProgressLog 组件（props `turn`；`<details :open="status==='streaming'">`；li 按 kind info/done/error 着色）
- [x] **R3** `components/answer-turn.js`：答案卡组件（error/result 变体、角标/论断/反馈/重试/复制/四个折叠）
- [x] **R4** CSS 三分拆：`app.css`（tokens+壳）/ `chat.css`（线程+composer+进度）/ `panels.css`（反馈/折叠/引用/树/路径）；各 ≤300 行
- [x] **R5** `vendor/README.md`：vendor 说明 + curl 命令 + 升级流程
- [x] **R6** `tests/unit/test_web_claude_ui.py` 重写：文件全集断言、Vue 根结构、注入安全零命中、API 流程
- [x] **R8** **ADR-006** 正式落 tech-stack.md（§1 前端行 → 已采纳；§3 待决策项勾选）
- [x] **R9** rules.md §8 改版 + 版本行 V1.0→V1.1；同步 CLAUDE.md Conventions
- [x] **R10** api-and-ui.md §2 升版 V1.3（技术形态表/增强项/验证清单）
- [x] **R11** EXTERNAL_RUNTIMES.md：Vue 3 运行时行 + 变更记录
- [x] **R12** 状态同步：IMPORTANT.md §5 → [x]；phase-5-scale.md P5-UI-01 → [x]；phase-4-pilot.md P4-UI 行加指针注；README 更新
- [x] **R13** 合入：单 changeset；提交信息 `feat(web): vue 3 zero-build refactor with interactive trial UI (P5-UI-01)`（无 Co-Authored-By）
