...
def _index(
    triples: list[Triple],
) -> tuple[list[_Edge], dict[str, list[_Edge]], dict[str, list[_Edge]]]:
    edges: list[_Edge] = []
    out_adj: dict[str, list[_Edge]] = defaultdict(list)
    in_adj: dict[str, list[_Edge]] = defaultdict(list)
    for t in triples:
        e = _Edge(
...
def generate_gold_cases(
    ...
    *,
    ...
    edges, out_adj, in_adj = _index(triples)
    cases: list[EvalCase] = []
    seen_q: set[str] = set()
    ...
    # --- 3-hop: CEO of competitor of producer of product ---
    n3 = 0
    for e in edges:
        if n3 >= max_3hop:
            break
    ...
    # parent of producer of product (2-hop also fills 3 when extended)
    for e in edges:
        if n3 >= max_3hop:
            break
        if e.relation != "PRODUCES":
    ...
    # --- open path: path / shared connections between two entities ---
    n_open = 0
    companies = sorted(
    ...
    # --- no-answer: fabricated entities not in graph ---
