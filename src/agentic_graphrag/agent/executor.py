...
class Executor:
    ...
    def run(
        ...
        *,
        ...
        # Cache keys are tenant-scoped (BL-04), so tenant-scoped runs no longer
        # have to bypass the cache to stay isolated.
        ...
        fused = fuse_and_cache(
            self, evidence, sub_question, tools_key=tools_key, tenant_id=tenant_id
        )
        return fused, traces
